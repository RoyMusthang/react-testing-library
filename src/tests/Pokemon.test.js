...
describe('Testa o component Pokemon', () => {
  ...
  it('Testa se o card contém o link de detalhes de acordo com o ID do pokemon', () => {
  ...
  it('Testa se existe um icone de estrela no pokemon favoritado', () => {
    renderWithRouter(<App />);
    const details = screen.getByRole('link', { name: /More details/i });
    userEvent.click(details);
    ...
    expect(favImg.src).toBe(`http://localhost${imageStar}`);
  });
});
